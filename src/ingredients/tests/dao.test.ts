...
describe("createIngredient", () => {
    it("should create an ingredient and return it", async () => {
        (IngredientModel.prototype.save as jest.Mock).mockResolvedValue(dummyIngredient);
...
describe("getIngredientById", () => {
    ...
    it("should throw an error if ingredient not found", async () => {
        ...
        await expect(getIngredientById("nonexistent-id")).rejects.toThrow("Ingredient not found");
    });
});

describe("updateIngredient", () => {
    it("should update an ingredient and return the updated data", async () => {
        (IngredientModel.findOneAndUpdate as jest.Mock).mockResolvedValue(dummyIngredient);
    ...
    it("should throw an error if ingredient not found", async () => {
        ...
        await expect(updateIngredient("nonexistent-id", dummyIngredient)).rejects.toThrow("Ingredient not found");
    });
});

describe("deleteIngredient", () => {
    it("should delete an ingredient and return the result", async () => {
        (IngredientModel.deleteOne as jest.Mock).mockResolvedValue({ deletedCount: 1 });
        const result = await deleteIngredient("1");
        expect(result).toEqual({ deletedCount: 1 });
    });

    it("should throw an error if ingredient not found", async () => {
        (IngredientModel.deleteOne as jest.Mock).mockResolvedValue({ deletedCount: 0 });
        await expect(deleteIngredient("nonexistent-id")).rejects.toThrow("Ingredient not found");
    });
});
