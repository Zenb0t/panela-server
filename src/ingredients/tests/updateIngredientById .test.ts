...
describe('updateIngredientById Middleware', () => {
  ...
  it('should update an ingredient and send a 200 response', async () => {
    ...
    const req = { params: { id: ingredientId }, body: updatedIngredientData } as unknown as Request;
    const res = mockResponse();

    await updateIngredientById(req, res, mockNext);

    expect(updateIngredient).toHaveBeenCalledWith(ingredientId, updatedIngredientData);
  ...
  it('should call handleError when the update fails', async () => {
    ...
    const req = { params: { id: dummyIngredient._id }, body: updatedIngredientData }as unknown as Request;
    const res = mockResponse();
